    ...
    serializer: {
      ...
      deserialize: (raw) => {
        ...
        const widgets = new Map(
          Object.entries(parsed.widgets).filter(
            ([_, widget]: any) =>
              widget.type in widgetRegistry
          )
        )
        return { widgets }
      }
...
const excludePatterns = ['BaseWidget', 'Velocity', 'Route2d', 'RollAndPitch', 'LcmSpy', 'Heading', 'GroundSpeed']
