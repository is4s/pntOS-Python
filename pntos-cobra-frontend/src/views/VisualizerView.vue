<template>
  ...
  <div class="legend">
    <div v-for="[sourceName, source] in sourceEntries" :key="sourceName" class="legend-row">
      <span class="legend-source">
        <span class="legend-dot" :style="{ background: source.color }" />
      ...
      <span class="legend-controls">
        <label>
          <input type="checkbox" :checked="mapStore.visibleSources[sourceName] ?? true"
                 @change="toggleSource(sourceName, $event)">
          Show
        </label>

        <label>
          <input type="radio" name="active-source" :checked="mapStore.centerSource === sourceName"
...
<script setup lang="ts">
  ...
  import { type PositionMessage, type PvaMessage } from '@/utils/dataStructures';
  ...
  const ublox_pos = useRegistry<PositionMessage>(UBLOX_GROUP, "message")
