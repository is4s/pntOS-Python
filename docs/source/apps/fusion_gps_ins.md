...
| {py:obj}`AspnLcmTransportPlugin <pntos.cobra.AspnLcmTransportPlugin>`               | Listens to a TCP relay for {term}`ASPN` messages from a {term}`LCM` log file and feeds these {term}`ASPN` messages into the app as pntOS {py:obj}`Message <pntos.api.Message>`s.                                                                                                                                                                                                                                                                                                              | [](../plugins/transport_plugin.md)       |
