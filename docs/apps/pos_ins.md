...
imports since the API is mostly relevant when _implementing_ a plugin, not when _using_
a plugin in an app.
...
| {py:obj}`ControllerConfig <pntos.cobra.config.ControllerConfig>`                      | Configuration for the controller plugin which includes the length of measurement buffers and the solution request rate.|
...
| {py:obj}`MountingConfig <pntos.cobra.config.MountingConfig>`                          | Information about the GPS antenna's relationship to the platform frame.                                                                                   |
...
| {py:obj}`UiLogPlottingConfig <pntos.cobra.config.UiLogPlottingConfig>`                | Configuration for the UI plotting plugin that defines the log file, and the truth and solution PVA channels to plot.
...
| {py:obj}`TimeBiasConfig <pntos.cobra.config.TimeBiasConfig>`                         | Configuration for a preprocessor that corrects for biased timestamps from the given channels.
...
> _"A function that will be called by pntOS once and only once when it first initializes
> the plugin before any other functions on the plugin are called."_
...
| {py:obj}`TutorialLcmLogTransportPlugin <pntos.cobra.TutorialLcmLogTransportPlugin>`               | Reads {term}`ASPN` messages from an {term}`LCM` log file and feeds these {term}`ASPN` messages into the app as pntOS {py:obj}`Message <pntos.api.Message>`s.                                                                                                                                                                                                                                                                                                              | [](../plugins/transport_plugin.md)       |
