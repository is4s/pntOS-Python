...
def get_ros_bag_file(specified_file: Path) -> Path:  # pragma: no cover
    actual_file = specified_file / f'{specified_file}_0.db3'
    if not actual_file.exists():
        actual_file = specified_file / f'{specified_file}_0.mcap'

    return actual_file


def run_ros_logger(output_file: Path) -> Popen[bytes]:  # pragma: no cover
...
def run_ros_bag_player(logfile: Path) -> Popen[bytes]:  # pragma: no cover
    return Popen(
