...
def test_ManualAlignmentConfig_to_and_from_registry(mediator) -> None:
    ...
    config_to_registry(test_conf, mediator)
    _validate_conf_to_registry(mediator, test_conf)

    # Test config_from_registry()
    result_conf = config_from_registry(
        ManualAlignmentConfig, mediator, CONFIG_TEST_GROUP
    )
    assert result_conf is not None

    _validate_conf_from_registry(mediator, test_conf, result_conf)


def test_ImuConfig_to_from_registry(mediator) -> None:
    test_conf = ImuConfig(
        CONFIG_TEST_GROUP,
        (2.1, 2.2, 2.3),
        (3.1, 3.2, 3.3),
    ...
    # Test config_from_registry()
    result_conf = config_from_registry(ImuConfig, mediator, CONFIG_TEST_GROUP)
    assert result_conf is not None

    _validate_conf_from_registry(mediator, test_conf, result_conf)


def test_MountingConfig_to_from_registry(mediator) -> None:
    ...
    # Test config_from_registry()
    result_conf = config_from_registry(MountingConfig, mediator, CONFIG_TEST_GROUP)
    assert result_conf is not None

    _validate_conf_from_registry(mediator, test_conf, result_conf)


def test_DownsamplerConfig_to_from_registry(mediator) -> None:
    ...
    result_conf = config_from_registry(DownsamplerConfig, mediator, CONFIG_TEST_GROUP)
    assert result_conf is not None
    _validate_conf_from_registry(mediator, test_conf, result_conf)


def test_PreprocessorConfig_to_from_registry(mediator) -> None:
    ...
    result_conf = config_from_registry(PreprocessorConfig, mediator, CONFIG_TEST_GROUP)
    assert result_conf is not None
    _validate_conf_from_registry(mediator, test_conf, result_conf)


def test_OrchestrationConfig_to_from_registry(mediator) -> None:
    C_imu_to_platform = (
    ...
    _validate_conf_from_registry(mediator, test_conf, result_config)


def test_static_align_config_to_from_registry(mediator) -> None:
    imu_model = ImuConfig(
        CONFIG_TEST_GROUP,
        (1.23, 2.34, 3.45),
        (12.3, 23.4, 34.5),
    ...
    result_config = config_from_registry(
        StaticAlignmentConfig, mediator, CONFIG_TEST_GROUP
    )
    assert result_config is not None
...
def test_get_correct_type_to_from_registry(mediator) -> None:
    imu_model = ImuConfig(
        CONFIG_TEST_GROUP,
        (1.23, 2.34, 3.45),
        (12.3, 23.4, 34.5),
    ...
    with mediator.registry.batch_start(CONFIG_TEST_GROUP) as kv:
...
def test_config_from_registry_return_none(mediator) -> None:
    ...
    reg = mediator.registry.batch_start(CONFIG_TEST_GROUP)
    reg['sigma'] = ['wrong type']
    reg.batch_end()

    # Test config_from_registry()
    result_conf = config_from_registry(FogmConfig, mediator, CONFIG_TEST_GROUP)
    assert result_conf is None


def test_manual_ewc_validation(mediator) -> None:
    ...
    ret_ewc = validate_manual_ewc(valid_ewc, 3, mediator)
    assert ret_ewc is not None
    _compare_ewc(ret_ewc, valid_ewc)
    # Test convertable dimensionality
    est = np.zeros((3,))
    cov = np.array([1, 1, 1])
    convertable_ewc = EstimateWithCovariance(type, est, cov)
    ret_ewc = validate_manual_ewc(convertable_ewc, 3, mediator)
    assert ret_ewc is not None
    _compare_ewc(ret_ewc, valid_ewc)
    ...
    assert (
        validate_manual_ewc(
            EstimateWithCovariance(type, np.zeros((3, 1, 3)), cov), 3, mediator
        )
        is None
    )
    assert (
        validate_manual_ewc(
            EstimateWithCovariance(type, est, np.zeros((3, 3, 3))), 3, mediator
        )
        is None
    )
    # Test state count mismatch for est and cov
    assert (
        validate_manual_ewc(EstimateWithCovariance(type, est, cov), 4, mediator) is None
    )
    assert (
        validate_manual_ewc(
            EstimateWithCovariance(type, np.zeros((4,)), cov), 4, mediator
        )
        is None
    )
    ...
    assert (
        validate_manual_ewc(
            EstimateWithCovariance(type, np.zeros((3, 2)), cov), 3, mediator
        )
        is None
    )
    assert (
        validate_manual_ewc(
            EstimateWithCovariance(type, est, np.zeros((3, 2))), 3, mediator
        )
        is None
    )


def _compare_ewc(ewc1: EstimateWithCovariance, ewc2: EstimateWithCovariance) -> None:
...
def _validate_conf_to_registry(mediator: Mediator, test_conf: BaseConfig) -> None:
    kv = mediator.registry.batch_start(CONFIG_TEST_GROUP)
...
def _validate_conf_from_registry(
    mediator: Mediator, test_conf: BaseConfig, result_conf: BaseConfig
) -> None:
    ...
    for test_field, result_field in zip(test_fields, result_fields, strict=True):
        ...
        if isinstance(test_val, np.ndarray):
            ...
            _compare_ewc(test_val, result_val)
...
def _make_config(
    field_name: str, field_type: type, optional: bool = False
) -> type[BaseConfig]:
...
def _create_dummy_value(in_type: type[Any]) -> Any:  # noqa: ANN401
...
def test_supported_types(mediator) -> None:
    for i in range(3):
        for j, t in enumerate(SUPPORTED_TYPES):
            dynamic_group = f'dynamic_test_{i}{j}'
            DynConf = _make_config('dynamic_field', t, optional=bool(i))
            # test with dummy value when type hint is and isn't optional; test with None
            val = _create_dummy_value(t) if i in {0, 2} else None
            conf = DynConf(
                dynamic_field=val,  # ty:ignore[unknown-argument]
                group=dynamic_group,
            )
            config_to_registry(conf, mediator)
            out_conf = config_from_registry(DynConf, mediator, dynamic_group)
            assert out_conf is not None
            _validate_conf_from_registry(mediator, conf, out_conf)


def test_int_to_float_conversion(mediator) -> None:
    group = 'itfconv'
    DynConf = _make_config('dynamic_field', float)
    val = 1
    conf = DynConf(dynamic_field=val, group=group)  # ty:ignore[unknown-argument]
    config_to_registry(conf, mediator)
    out_conf = config_from_registry(DynConf, mediator, group)
    assert out_conf is not None
    assert conf.dynamic_field == out_conf.dynamic_field  # ty:ignore[unresolved-attribute]


def test_int_to_optional_float_conversion(mediator) -> None:
    group = 'itf_opt_conv'
    DynConf = _make_config('dynamic_field', float, True)
    val = 1
    conf = DynConf(dynamic_field=val, group=group)  # ty:ignore[unknown-argument]
    config_to_registry(conf, mediator)
    out_conf = config_from_registry(DynConf, mediator, group)
...
def test_list_to_tuple_conversion(mediator) -> None:
    group = 'md_list_to_tuple'
    DynConf = _make_config(
    ...
    DynConf = _make_config('dynamic_field', tuple[str, ...])
    new_val = ['hello', 'world']
...
def test_ndarray_to_tuple_conversion(mediator) -> None:
...
def test_non_uniform_tuple(mediator) -> None:
    group = 'non_uniform'
    DynConf = _make_config(
...
def test_multi_dim_str_tuple(mediator) -> None:
    group = 'md_tuple_of_str'
    DynConf = _make_config('dynamic_field', tuple[tuple[str, ...], tuple[str, ...]])
...
def test_optional_config(mediator) -> None:
    ...
    config_to_registry(in_foo, mediator)
    out_foo = config_from_registry(Foo, mediator, 'foo')
    assert out_foo is not None
    _validate_conf_from_registry(mediator, in_foo, out_foo)
    ...
    config_to_registry(in_bar, mediator)
    out_bar = config_from_registry(Bar, mediator, 'bar')
    assert out_bar is not None
    _validate_conf_from_registry(mediator, in_bar, out_bar)


def test_multiple_types_produces_error(mediator) -> None:
    @dataclass
    class Foo(BaseConfig):
        foo: int | str | None
...
def test_warn_on_overwrite(mediator, capsys: pytest.CaptureFixture[str]) -> None:
    @dataclass
    class Foo(BaseConfig):
        foo: list[str]
    ...
    config_to_registry(_in_foo_1, mediator)

    # should create overwrite warning
    config_to_registry(_in_foo_2, mediator)
