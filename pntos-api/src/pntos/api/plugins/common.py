...
"""
A ``TypeVar`` of the types allowed in :class:`KeyValueStore`. 
...
Example:
    For example, :meth:`KeyValueStore.get_value` needs to guarantee that
    the input and the return types are the same. Thus, :meth:`get_value` would
...
"""
This is a union of all types allowed in :class:`KeyValueStore`. 

This is particularly for cases where a method does not need to guarantee that
the type on an input is the same as the returned type.

Example:
    For example, :meth:`KeyValueStore.set_value` does not need to guarantee that
...
class KeyValueStore(ABC):
    ...
    def get_value(
        ...
        Returns:
            ...
            :meth:`__contains__`) and if the store can convert the value to the requested type.
        """
        pass
    ...
    def get_raw(self, key: str | None = None) -> bytes | None:
        ...
        Returns:
            ...
            checked with :meth:`__contains__`::
    ...
    def get_type(self, key: str) -> type[RegistryValueTypeUnion] | None:
        ...
        Returns:
            ...
            this is guaranteed to return the return type of :meth:`__getitem__`
