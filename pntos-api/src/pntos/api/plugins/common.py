...
class EstimateWithCovarianceType(Enum):
    ...
    Contains a mean (estimate) and covariance describing a multivariate Gaussian distribution.
      ...
      - `EstimateWithCovariance.estimate` is size Nx1 where N is the length field.
      - `EstimateWithCovariance.covariance` is size NxN where N is the length field.
    """
    ...
    error form.
      ...
      - `EstimateWithCovariance.covariance` is size 3x3, in :math:`\\text{radians}^2`.
    """
...
class FusionType(Enum):
    ...
    Example:
        Suppose we have a variable `FusionPlugin* plugin`. Then if the return value of
        `plugin->is_fusion_type_supported(plugin, STANDARD_MODEL)` is true, then that means that
...
class KeyValueStore(Protocol):
    ...
    def batch_restart(self) -> None:
        ...
        Note: 
            ...
            should endeavour to call `batch_end` as soon as possible after they are done
    ...
    def request_notify(
        ...
        Note:
            The callback must not attempt to set any values inside the KeyValueStore, as
            ...
            possible. Calling `Mediator` within the callback may be disallowed by the
...
class Registry(Protocol):
    ...
    def batch_start(self, group: str) -> KeyValueStore:
        ...
        Note:
            While a batch is active, access to the store may be denied to other users. Thus a user
            should endeavour to call `KeyValueStore.batch_end` as soon as possible after they are
            done getting/setting values in the returned `KeyValueStore`.
...
class CommonPlugin(Protocol):
    ...
    Example:
        ...
        fields on the `TransportPlugin` class. Thus, the fields of the `CommonPlugin`
        nested on the `TransportPlugin` are implemented by the plugin writer.
    ...
    def init_plugin(
        ...
        Note:
            ...
            `Mediator` class, and abstracts away the return communication channel from the plugin to
            the rest of the system. Thus, the plugin need only implement `Mediator` by simply saving
            a copy of the functions that the controller passes into it. Then, when the plugin later
            needs to make requests of the system, it may call a function in its copy of `Mediator`,
            without needing any knowledge of how the controller implemented `Mediator`. This allows
            controllers to implement arbitrary concurrency models, including single-threaded,
            multi-threaded, multi-process, and distributed computing.
